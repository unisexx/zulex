...
<script type="text/javascript">
			$(document).ready( function() {
				// Default options
				$("#accessories, #softwares, #brochures, #wallpapers").multiSelect();
			});			
</script>
<script type="text/javascript">
	$(function(){
		$("input[type=button]").live('click',function(){
			...
			'<tr>' +
			'<th></th>' +
			'<td><?php echo form_upload('file[]','','size=72')?> <input type="button" value="เพิ่ม" /></td>' +
			'</tr>'
			);
		})
	})
...
<table class="list">
	<tr>
		<th><a href="products/admin/categories/index">สินค้า</a> » <a href="products/admin/categories/sub_categories/<?php echo $parent->id?>"><?php echo lang_decode($parent->name)?></a> » <?php echo lang_decode($category->name)?>
	</tr>
</table>
<br>
<form method="post" action="products/admin/products/common_save/<?php echo $parent->id?>/<?php echo $product->id?>" id="frm" enctype="multipart/form-data">
<table class="form tab">
	...
	<tr>
		...
		<td>
			...
			<?php if($product->before):?>
				...
				<a href="products/admin/products/remove_before/<?php echo $parent->id?>/<?php echo $category->id?>/<?php echo $category->id?>/<?php echo $product->id?>" onclick="return confirm('ต้องการลบรูปนี้')" /><span class="icon icon-cross"></span></a>
			<?php endif;?>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<?php if($product->after):?>
				...
				<a  href="products/admin/products/remove_after/<?php echo $parent->id?>/<?php echo $category->id?>/<?php echo $category->id?>/<?php echo $product->id?>" onclick="return confirm('ต้องการลบรูปนี้')" /><span class="icon icon-cross"></span></a>
			<?php endif;?>
		</td>
	</tr>
	-->
    ...
    <tr>
		<th>หมวด :</th>
		<td>
			<?php echo form_dropdown('category_id',$product->category->get_option('parents = '.$category->parents),$this->uri->segment(6, 0),'');?>
        </td>
	</tr>
	<tr>
		<th>โค้ดวิดีโอ :</th>
					...
					<?php foreach ($categories as $category):?>
						<optgroup label="<?php echo lang_decode($category->name)?>">
							<?php foreach ($category->product->order_by('id','desc')->get() as $product_accessories):?>
                                ...
                                //if(ereg("$product_accessories->id+", "$product->accessories")){
                                    //echo "selected=selected";
                                //}
                                ?>
